...
<head>
    <title>Code coverage report for compiler/DashboardTemplateEvaluator.js</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="../index.html">All files</a> / <a href="index.html">compiler</a> DashboardTemplateEvaluator.js</h1>
        <div class='clearfix'>
...
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-yes">1x</span>
<span class="cline-any cline-neutral">&nbsp;</span></td><td class="text"><pre class="prettyprint lang-js">const Joi = require('@hapi/joi');
...
<span class="cstat-no" title="statement not covered" >        errorReporter.error(err.message);</span>
      } else {
...
&nbsp;
  compileTemplate(dashboardTemplate, errorReporter) {
    return {
      ...dashboardTemplate,
      title: dashboardTemplate.title || <span class="branch-1 cbranch-no" title="branch not covered" >inlection.titleize(dashboardTemplate.name),</span>
      items: (dashboardTemplate.items || <span class="branch-1 cbranch-no" title="branch not covered" >[])</span>.map(item =&gt; this.compileItem(item, errorReporter))
    };
  }
&nbsp;
  compileItem(itemTemplate, errorReporter) {
    <span class="missing-if-branch" title="if path not taken" >I</span>if (!itemTemplate.measures &amp;&amp; <span class="branch-1 cbranch-no" title="branch not covered" >!itemTemplate.dimensions)</span> {
<span class="cstat-no" title="statement not covered" >      errorReporter.error(`Either measures or dimensions should be declared for valid query`);</span>
<span class="cstat-no" title="statement not covered" >      return null;</span>
    ...
    <span class="missing-if-branch" title="else path not taken" >E</span>if (itemTemplate.measures) {
      config.metrics = this.cubeEvaluator.evaluateReferences(null, itemTemplate.measures);
    }
    <span class="missing-if-branch" title="else path not taken" >E</span>if (itemTemplate.dimensions) {
      config.dimension = this.cubeEvaluator.evaluateReferences(null, itemTemplate.dimensions);
    }
    <span class="missing-if-branch" title="if path not taken" >I</span>if (itemTemplate.segments) {
<span class="cstat-no" title="statement not covered" >      config.segments = this.cubeEvaluator.evaluateReferences(null, itemTemplate.segments);</span>
    }
    <span class="missing-if-branch" title="else path not taken" >E</span>if (itemTemplate.order) {
      config.order = itemTemplate.order.map(f =&gt; ({
        desc: f.direction === 'desc',
        id: this.cubeEvaluator.evaluateReferences(null, f.member)
      }));
    }
    <span class="missing-if-branch" title="else path not taken" >E</span>if (itemTemplate.filters) {
    ...
    <span class="missing-if-branch" title="else path not taken" >E</span>if (itemTemplate.timeDimension) {
