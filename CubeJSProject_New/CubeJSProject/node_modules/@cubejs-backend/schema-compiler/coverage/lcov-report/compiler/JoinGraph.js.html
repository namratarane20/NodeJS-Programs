...
<head>
    <title>Code coverage report for compiler/JoinGraph.js</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="../index.html">All files</a> / <a href="index.html">compiler</a> JoinGraph.js</h1>
        <div class='clearfix'>
    ...
    return R.compose(
      ...
      R.map(join =&gt; {
        const multipliedMeasures = R.compose(
          R.filter(
<span class="fstat-no" title="function not covered" >            m </span>=&gt; <span class="cstat-no" title="statement not covered" >m.sql &amp;&amp; this.cubeEvaluator.funcArguments(m.sql).length === 0 &amp;&amp; m.sql() === 'count(*)' ||</span>
            ['sum', 'avg', 'count', 'number'].indexOf(m.type) !== -1
          ),
          R.values
        );
        const joinRequired =
<span class="fstat-no" title="function not covered" >          (v</span>) =&gt; <span class="cstat-no" title="statement not covered" >`primary key for '${v}' is required when join is defined in order to make aggregates work properly`;</span>
        <span class="missing-if-branch" title="if path not taken" >I</span>if (
          !this.cubeEvaluator.primaryKeys[join[1].from] &amp;&amp;
<span class="branch-1 cbranch-no" title="branch not covered" >          multipliedMeasures(this.cubeEvaluator.measuresForCube(join[1].from)).length &gt; 0</span>
        ) {
<span class="cstat-no" title="statement not covered" >          errorReporter.error(joinRequired(join[1].from));</span>
<span class="cstat-no" title="statement not covered" >          return null;</span>
        }
        <span class="missing-if-branch" title="if path not taken" >I</span>if (!this.cubeEvaluator.primaryKeys[join[1].to] &amp;&amp;
<span class="branch-1 cbranch-no" title="branch not covered" >          multipliedMeasures(this.cubeEvaluator.measuresForCube(join[1].to)).length &gt; 0)</span> {
<span class="cstat-no" title="statement not covered" >          errorReporter.error(joinRequired(join[1].to));</span>
<span class="cstat-no" title="statement not covered" >          return null;</span>
        }
        return join;
      }),
      ...
      R.filter(R.identity),
      R.map(join =&gt; {
        <span class="missing-if-branch" title="if path not taken" >I</span>if (!this.cubeEvaluator.cubeExists(join[0])) {
<span class="cstat-no" title="statement not covered" >          errorReporter.error(`Cube ${join[0]} doesn't exist`);</span>
...
&nbsp;
<span class="fstat-no" title="function not covered" >  bu</span>ildJoinNode(cube) {
<span class="cstat-no" title="statement not covered" >    return R.compose(</span>
      R.fromPairs,
      R.map(<span class="fstat-no" title="function not covered" >v </span>=&gt; <span class="cstat-no" title="statement not covered" >[v[0], 1])</span>,
      R.toPairs
    )(cube.joins || {});
  }
&nbsp;
  buildJoin(cubesToJoin) {
    ...
    <span class="missing-if-branch" title="if path not taken" >I</span>if (!join) {
<span class="cstat-no" title="statement not covered" >      throw new UserError(`Can't find join path to join ${cubesToJoin.map(<span class="fstat-no" title="function not covered" >v </span>=&gt; <span class="cstat-no" title="statement not covered" >`'${v}'`)</span>.join(', ')}`);</span>
    }
