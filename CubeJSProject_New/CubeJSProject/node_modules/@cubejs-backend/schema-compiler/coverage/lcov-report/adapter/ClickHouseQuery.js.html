...
<head>
    <title>Code coverage report for adapter/ClickHouseQuery.js</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="../index.html">All files</a> / <a href="index.html">adapter</a> ClickHouseQuery.js</h1>
        <div class='clearfix'>
...
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-yes">1x</span>
<span class="cline-any cline-neutral">&nbsp;</span></td><td class="text"><pre class="prettyprint lang-js">const BaseQuery = require('./BaseQuery');
...
class ClickHouseFilter extends BaseFilter {
  likeIgnoreCase(column, not) {
    return `lower(${column}) ${not ? <span class="branch-0 cbranch-no" title="branch not covered" >'NOT' </span>: ''} LIKE CONCAT('%', lower(?), '%')`;
  }
}
&nbsp;
&nbsp;
...
&nbsp;
  timeGroupedColumn(granularity, dimension) {
    <span class="missing-if-branch" title="if path not taken" >I</span>if (granularity === 'week') {
<span class="cstat-no" title="statement not covered" >      return `toDateTime(toMonday(${dimension}, '${this.timezone}'), '${this.timezone}')`;</span>
    } else {
...
&nbsp;
<span class="fstat-no" title="function not covered" >  ca</span>lcInterval(operation, date, interval) {
    const [intervalValue, intervalUnit] = <span class="cstat-no" title="statement not covered" >interval.split(" ");</span>
    // eslint-disable-next-line prefer-template
    const fn = <span class="cstat-no" title="statement not covered" >operation + intervalUnit[0].toUpperCase() + intervalUnit.substring(1) + "s";</span>
<span class="cstat-no" title="statement not covered" >    return `${fn}(${date}, ${intervalValue})`;</span>
  }
&nbsp;
<span class="fstat-no" title="function not covered" >  su</span>btractInterval(date, interval) {
<span class="cstat-no" title="statement not covered" >    return this.calcInterval("subtract", date, interval);</span>
  }
&nbsp;
<span class="fstat-no" title="function not covered" >  ad</span>dInterval(date, interval) {
<span class="cstat-no" title="statement not covered" >    return this.calcInterval("add", date, interval);</span>
  }
&nbsp;
  timeStampCast(value) {
...
&nbsp;
    ...
    // However parseDateTimeBestEffort works with ISO8601
    //
<span class="cstat-no" title="statement not covered" >    return `parseDateTimeBestEffort(${value})`;</span>
  }
&nbsp;
  getFieldAlias(id) {
...
&nbsp;
    <span class="missing-if-branch" title="else path not taken" >E</span>if (field) {
      return field.aliasName();
    }
&nbsp;
...
&nbsp;
  primaryKeyCount(cubeName, distinct) {
    ...
    <span class="missing-if-branch" title="else path not taken" >E</span>if (distinct) {
      return `uniqExact(${primaryKeySql})`;
    } else {
<span class="cstat-no" title="statement not covered" >      return `count(${primaryKeySql})`;</span>
    }
  }
&nbsp;
&nbsp;
...
&nbsp;
   ...
   */
&nbsp;
    const datesFrom = <span class="cstat-no" title="statement not covered" >[];</span>
    const datesTo = <span class="cstat-no" title="statement not covered" >[];</span>
<span class="cstat-no" title="statement not covered" >    timeDimension.timeSeries().forEach(<span class="fstat-no" title="function not covered" >([</span>from, to]) =&gt; {</span>
<span class="cstat-no" title="statement not covered" >      datesFrom.push(from);</span>
<span class="cstat-no" title="statement not covered" >      datesTo.push(to);</span>
    });
<span class="cstat-no" title="statement not covered" >    return `SELECT parseDateTimeBestEffort(arrayJoin(['${datesFrom.join("','")}'])) as date_from, parseDateTimeBestEffort(arrayJoin(['${datesTo.join("','")}'])) as date_to`;</span>
  }
&nbsp;
