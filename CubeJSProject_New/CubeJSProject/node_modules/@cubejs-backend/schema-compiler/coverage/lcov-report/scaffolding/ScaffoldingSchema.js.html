...
<head>
    <title>Code coverage report for scaffolding/ScaffoldingSchema.js</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="../index.html">All files</a> / <a href="index.html">scaffolding</a> ScaffoldingSchema.js</h1>
        <div class='clearfix'>
            ...
            <div class='fl pad1y space-right2'>
                ...
                <span class='fraction'>65/80</span>
            </div>
...
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-yes">1x</span>
<span class="cline-any cline-neutral">&nbsp;</span></td><td class="text"><pre class="prettyprint lang-js">const inflection = require('inflection');
...
&nbsp;
  resolveTableDefinition(tableName) {
    const [schema, table] = this.parseTableName(tableName);
    <span class="missing-if-branch" title="if path not taken" >I</span>if (!this.dbSchema[schema]) {
<span class="cstat-no" title="statement not covered" >      throw new UserError(`Can't resolve ${tableName}: '${schema}' does not exist`);</span>
    }
    <span class="missing-if-branch" title="if path not taken" >I</span>if (!this.dbSchema[schema][table]) {
<span class="cstat-no" title="statement not covered" >      throw new UserError(`Can't resolve ${tableName}: '${table}' does not exist`);</span>
    }
...
&nbsp;
    return {
      ...
      joins: includeJoins ? this.joins(tableName, tableDefinition) : <span class="branch-1 cbranch-no" title="branch not covered" >[]</span>
    };
  }
&nbsp;
  parseTableName(tableName) {
    const schemaAndTable = tableName.split('.');
    <span class="missing-if-branch" title="if path not taken" >I</span>if (schemaAndTable.length !== 2) {
<span class="cstat-no" title="statement not covered" >      throw new UserError(`Incorrect format for '${tableName}'. Should be in '&lt;schema&gt;.&lt;table&gt;' format`);</span>
    }
...
&nbsp;
      <span class="missing-if-branch" title="else path not taken" >E</span>if (column.columnType !== 'time') {
...
&nbsp;
    const timeColumns = R.pipe(
      ...
      R.sortBy(<span class="fstat-no" title="function not covered" >co</span>lumn =&gt; <span class="cstat-no" title="statement not covered" >this.timeColumnIndex(column))</span>,
      R.map(<span class="fstat-no" title="function not covered" >co</span>lumn =&gt; (<span class="cstat-no" title="statement not covered" >{ ...column, columnType: 'time' })</span>) // TODO do we need it?
    )(tableDefinition);
&nbsp;
...
&nbsp;
        <span class="missing-if-branch" title="if path not taken" >I</span>if (!tablesToJoin) {
<span class="cstat-no" title="statement not covered" >          return null;</span>
        }
&nbsp;
        const columnsToJoin = tablesToJoin.map(definition =&gt; {
          <span class="missing-if-branch" title="if path not taken" >I</span>if (tableName === definition.tableName) {
<span class="cstat-no" title="statement not covered" >            return null;</span>
          }
          ...
          <span class="missing-if-branch" title="if path not taken" >I</span>if (!columnForJoin) {
<span class="cstat-no" title="statement not covered" >            return null;</span>
          }
          return {
...
&nbsp;
<span class="fstat-no" title="function not covered" >  ti</span>meColumnIndex(column) {
    const name = <span class="cstat-no" title="statement not covered" >column.name.toLowerCase();</span>
<span class="cstat-no" title="statement not covered" >    if (name.indexOf('create') !== -1) {</span>
<span class="cstat-no" title="statement not covered" >      return 0;</span>
    } else <span class="cstat-no" title="statement not covered" >if (name.indexOf('update') !== -1) {</span>
...
&nbsp;
  columnType(column) {
    ...
    <span class="missing-if-branch" title="if path not taken" >I</span>if (['time', 'date'].find(t =&gt; type.indexOf(t) !== -1)) {
