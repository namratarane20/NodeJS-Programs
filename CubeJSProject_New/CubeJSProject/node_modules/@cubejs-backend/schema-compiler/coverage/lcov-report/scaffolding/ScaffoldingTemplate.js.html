...
<head>
    <title>Code coverage report for scaffolding/ScaffoldingTemplate.js</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="../index.html">All files</a> / <a href="index.html">scaffolding</a> ScaffoldingTemplate.js</h1>
        <div class='clearfix'>
...
&nbsp;
  ...
  resolveTableName(tableName) {
    ...
    <span class="missing-if-branch" title="else path not taken" >E</span>if (tableParts.length === 2) {
      this.scaffoldingSchema.resolveTableDefinition(tableName);
      return tableName;
    } else <span class="cstat-no" title="statement not covered" >if (tableParts.length === 1) {</span>
      const schema = <span class="cstat-no" title="statement not covered" >Object.keys(this.dbSchema).find(</span>
<span class="fstat-no" title="function not covered" >        ta</span>bleSchema =&gt; <span class="cstat-no" title="statement not covered" >this.dbSchema[tableSchema][tableName] ||</span>
          this.dbSchema[tableSchema][inflection.tableize(tableName)]
      );
<span class="cstat-no" title="statement not covered" >      if (!schema) {</span>
<span class="cstat-no" title="statement not covered" >        throw new UserError(`Can't find any table with '${tableName}' name`);</span>
      }
<span class="cstat-no" title="statement not covered" >      if (this.dbSchema[schema][tableName]) {</span>
<span class="cstat-no" title="statement not covered" >        return `${schema}.${tableName}`;</span>
      }
<span class="cstat-no" title="statement not covered" >      if (this.dbSchema[schema][inflection.tableize(tableName)]) {</span>
<span class="cstat-no" title="statement not covered" >        return `${schema}.${inflection.tableize(tableName)}`;</span>
      }
    } else {
<span class="cstat-no" title="statement not covered" >      throw new UserError(`Table names should be in &lt;table&gt; or &lt;schema&gt;.&lt;table&gt; format`);</span>
    }
  }
&nbsp;
