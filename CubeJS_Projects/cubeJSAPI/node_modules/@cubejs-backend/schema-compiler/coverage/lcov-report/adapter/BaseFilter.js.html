...
<head>
    <title>Code coverage report for adapter/BaseFilter.js</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="../index.html">All files</a> / <a href="index.html">adapter</a> BaseFilter.js</h1>
        <div class='clearfix'>
...
<span class="cline-any cline-yes">1250x</span>
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-yes">51x</span>
<span class="cline-any cline-no">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span>
...
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-yes">1x</span>
<span class="cline-any cline-neutral">&nbsp;</span></td><td class="text"><pre class="prettyprint lang-js">const inlection = require('inflection');
...
&nbsp;
  ...
  measureFilterToWhere() {
    ...
    <span class="missing-if-branch" title="else path not taken" >E</span>if (measureDefinition.filters &amp;&amp; measureDefinition.filters.length ||
<span class="branch-2 cbranch-no" title="branch not covered" >      measureDefinition.drillFilters </span>&amp;&amp; <span class="branch-3 cbranch-no" title="branch not covered" >measureDefinition.drillFilters.length)</span> {
      return this.query.evaluateFiltersArray(
        (measureDefinition.filters || <span class="branch-1 cbranch-no" title="branch not covered" >[])</span>.concat(measureDefinition.drillFilters || []),
...
&nbsp;
  conditionSql(columnSql) {
    const operatorMethod = `${inlection.camelize(this.operator).replace(
      /[A-Z]/,
      (c) =&gt; (c != null ? c : <span class="branch-1 cbranch-no" title="branch not covered" >'')</span>.toLowerCase()
    )}Where`;
...
&nbsp;
  ...
  escapeWildcardChars(param) {
    return typeof param === 'string' ? param.replace(/([_%])/gi, '\\$1') : <span class="branch-1 cbranch-no" title="branch not covered" >param;</span>
  }
&nbsp;
  isWildcardOperator() {
...
&nbsp;
  filterParams() {
    ...
    <span class="missing-if-branch" title="if path not taken" >I</span>if (this.operator === 'set' || this.operator === 'not_set' || this.operator === 'expressionEquals') {
...
&nbsp;
  valuesArray() {
    return Array.isArray(this.values) ? this.values : <span class="branch-1 cbranch-no" title="branch not covered" >[this.values];</span>
  }
&nbsp;
  valuesContainNull() {
...
&nbsp;
<span class="fstat-no" title="function not covered" >  no</span>tContainsWhere(column) {
<span class="cstat-no" title="statement not covered" >    return this.likeOr(column, true);</span>
  }
&nbsp;
  likeOr(column, not) {
    const basePart = this.likeIgnoreCase(column, not);
    return `${join(not ? <span class="branch-0 cbranch-no" title="branch not covered" >' AND ' </span>: ' OR ', repeat(basePart, this.filterParams().length))}${this.orIsNullCheck(column, not)}`;
  }
&nbsp;
...
&nbsp;
  likeIgnoreCase(column, not) {
    return `${column}${not ? <span class="branch-0 cbranch-no" title="branch not covered" >' NOT' </span>: ''} ILIKE '%' || ? || '%'`;
  }
&nbsp;
  equalsWhere(column) {
...
&nbsp;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (this.valuesContainNull()) {
<span class="cstat-no" title="statement not covered" >      return this.notSetWhere(column);</span>
    }
&nbsp;
...
&nbsp;
  inPlaceholders() {
    return `(${join(', ', repeat(this.castParameter(), this.filterParams().length || <span class="branch-1 cbranch-no" title="branch not covered" >1)</span>)})`;
  }
&nbsp;
  inWhere(column) {
...
&nbsp;
  notEqualsWhere(column) {
    <span class="missing-if-branch" title="if path not taken" >I</span>if (this.isArrayValues()) {
<span class="cstat-no" title="statement not covered" >      return this.notInWhere(column);</span>
    }
&nbsp;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (this.valuesContainNull()) {
<span class="cstat-no" title="statement not covered" >      return this.setWhere(column);</span>
    }
&nbsp;
    return `${column} &lt;&gt; ${this.castParameter()}${this.orIsNullCheck(column, true)}`;
  }
&nbsp;
<span class="fstat-no" title="function not covered" >  no</span>tInWhere(column) {
<span class="cstat-no" title="statement not covered" >    return `${column} NOT IN ${this.inPlaceholders()}${this.orIsNullCheck(column, true)}`;</span>
  }
&nbsp;
<span class="fstat-no" title="function not covered" >  se</span>tWhere(column) {
<span class="cstat-no" title="statement not covered" >    return `${column} IS NOT NULL`;</span>
  }
&nbsp;
<span class="fstat-no" title="function not covered" >  no</span>tSetWhere(column) {
<span class="cstat-no" title="statement not covered" >    return `${column} IS NULL`;</span>
  }
&nbsp;
  gtWhere(column) {
...
&nbsp;
<span class="fstat-no" title="function not covered" >  lt</span>Where(column) {
<span class="cstat-no" title="statement not covered" >    return `${column} &lt; ${this.castParameter()}`;</span>
  }
&nbsp;
<span class="fstat-no" title="function not covered" >  lt</span>eWhere(column) {
<span class="cstat-no" title="statement not covered" >    return `${column} &lt;= ${this.castParameter()}`;</span>
  }
&nbsp;
<span class="fstat-no" title="function not covered" >  ex</span>pressionEqualsWhere(column) {
<span class="cstat-no" title="statement not covered" >    return `${column} = ${this.values[0]}`;</span>
  }
&nbsp;
